...
/**
 * Adapter where the favorite list is built
 */
class FavoriteAdapter(private val context: Context,
                      private val dataList: ArrayList<Gist>,
                      private val recyclerItemClickListener: RecyclerItemClickListener) : RecyclerView.Adapter<FavoriteAdapter.ViewHolder>() {
     ...
     * @param position     favorite item position
     */
    ...
    /**
     * In favorite list, the action is send to garbage the item, when user decided it
     *
     * @param holder       view holder where fields are into
     * @param gist     favorite item
     */
    fun setActionButton(holder: ViewHolder, gist: Gist) {
        Glide.with(context).load(R.drawable.ic_trash).into(holder.item_list_action)
    ...
    class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        var item_list_avatar = itemView.item_list_avatar
        var item_list_name =  itemView.item_list_name
        var item_list_type =  itemView.item_list_type
        var item_list_action =  itemView.item_list_action
    }
}
